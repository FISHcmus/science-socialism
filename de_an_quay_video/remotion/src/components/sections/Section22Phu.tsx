...
export const Section22Phu: FC = memo(() => {
  ...
  const statSprings = stats.map((_, i) => {
    const s = spring({ frame: Math.max(0, frame - 360 - i * 60), fps, config: { damping: 14, stiffness: 60 } });
    return interpolate(s, [0, 1], [0, 1]);
  });
  const statCountUps = stats.map((st, i) => {
    const localF = Math.max(0, frame - 360 - i * 60);
    ...
    <AbsoluteFill>
      {frame < 360 && (
        <AbsoluteFill className="flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(247,243,238,0.97) 0%, rgba(237,232,224,0.95) 100%)" }}>
          <SectionTitle title="Đặc điểm tôn giáo" subtitle="ở Việt Nam" sectionNumber="PHẦN 2.2" opacity={titleOpacity} translateY={titleTranslateY} accentWidth={titleAccentWidth} scale={titleScale} />
        </AbsoluteFill>
      )}
